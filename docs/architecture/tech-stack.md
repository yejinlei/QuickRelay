...
权威容量口径引用 [YEJ-135]：单机 **500 路用户视频**、**2 Gbps 双向吞吐容量**、分配类消息 **P99 < 1 ms（UDP 路径）**；TLS over TCP 路径 **P99 < 5 ms**（不适用 1 ms 承诺）。

> **v2 校准（[YEJ-167]，2026-09-16）**：架构原文「按「远超 500 路」设计」的余量口径已由需求方 2026-09-15 15:19 拍板落定为**单实例 allocation 上限 ~5,000（≈ 10× 500 路抗突发），状态表常驻 ~16 MB**。架构按「远超 500 路」设计的方式不变（无锁转发内核、零拷贝、按核分片状态表），**但 10× 是单实例余量、不是目标上限**，且是压测后可上调项（拐点由 [YEJ-151] 记录）。横向扩容靠多实例叠加，口径见架构文档 §7.4。
...
「引用 RFC」的含义：以规范文本为唯一行为来源，不复制 coturn 的 C 代码或任何脚本（与需求方已确认的 licensing 口径一致）。许可边界按**最严格解释**执行——不拷贝、不 vendor、只读取行为规格并转写；上游实际许可证的具体名称不作本文档的决策依据（见架构文档 §11.2 OQ-4）。
...
3. 合规：完全规避上游代码的许可传染风险（无论上游实际采用哪种许可证，边界一律按最严格解释执行），与需求方确认的「不复用 coturn 代码」口径一致。
...
- **REST 动态变更**：需求方已确认必须支持，且**仅本次运行内生效，重启回落静态配置，不落外部数据库、不建持久化审计库**。设计为 `ConfigController`（详见架构文档 §9）。
  - **可临时变更 4 类**（架构文档 §9.2 逐项列全并标注生效粒度）：限速与容量（`max-bps` / `max-bw` / `min-timeout` / `max-timeout` / `max-alloc-lifetime`）、流开关（`no-peer` / `no-data` / `no-channel`）、认证（realm / 静态密钥 / ephemeral 开关 / 追加临时 `user:pass`）、可观测性（日志级别 / 指标开关）。
  - **回滚**：`DELETE /api/v1/config/{key}` 单键回滚到启动值；**凭证类键（`users` / `static-secret` / `realm`）返回 409**（架构文档 §9.4）。
  - **不可通过 REST 修改（必须重启）**：监听地址与端口、relay-range / relay-ip、证书路径、认证模式、REST 自身监听配置。理由沿用架构文档 §9.3：这些变更会破坏状态表不变式（如已分配的 relay 地址与新 range 冲突）。
  - **鉴权双支持**：Bearer token（`Authorization: Bearer <token>`，`--api-token`）与 HMAC 请求签名（`X-QuickRelay-Date` + `X-QuickRelay-Signature`，`--rest-secret`）都实现，可各自开关、可同时启用；同时启用时 **Bearer 优先**（判定顺序见架构文档 §9.5）。新增配置项与 [YEJ-147] 的参数命名一致。
...
| 全局大锁状态表 | 否决 | 与按核分片 + 单包零同步的 P99 目标直接冲突（单实例 ~5,000 allocation 上限，架构文档 §7.1）；改为按核分片 + 分片内无锁 |
