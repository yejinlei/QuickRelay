...
//! where Binding is `0x0000`, so the `0x0001` / `0x0101` headers printed by
//! RFC 5769 decode as a Refresh request and success response. They are not
//! rejected, and the RFC 5769 datagrams parse byte for byte.

use bytes::Bytes;

use crate::attribute::{
    attribute_kind, is_comprehension_optional, is_trailer_only, parse_attribute_value,
    Attribute, AttrCode, AttributeKind,
};
use crate::error::{Error, ErrorKind};
use crate::fingerprint::MAX_ATTRIBUTE_COUNT;
use crate::integrity::AttributeLocation;
...
mod tests {
    ...
    fn rfc5769_2_1_parses_byte_exact() {
        ...
        assert_eq!(
            ...
            Some(&Attribute::MessageIntegrity([
                ...
                0xd3, 0xf2, 0x49, 0xc1, 0x17, 0x57, 0x1a
            ]))
        );
    ...
    fn rfc5769_2_2_parses_byte_exact() {
        ...
        assert_eq!(msg.fingerprint_offset(), Some(64));
        assert!(msg.has_message_integrity());
        assert!(msg.has_integrity());
        assert!(msg.has_fingerprint());
        assert!(msg.integrity().is_some());
    ...
    fn rfc5769_2_3_parses_byte_exact() {
        ...
        assert_eq!(msg.len(), 3);
        assert!(!msg.is_empty());
        assert_eq!(msg.get(0), Some(&Attribute::XorMappedAddress(MappedAddress::from_ipv6(
            [
        ...
        assert_eq!(
            msg.get(1),
            Some(&Attribute::MessageIntegrity([
                0xa3, 0x82, 0x95, 0x4e, 0x4b, 0x67, 0xbf, 0x11, 0x78, 0x4c, 0x97, 0xc8, 0x29,
                0x2c, 0x27, 0x5b, 0xfe, 0x3e, 0xd4, 0x01
            ]))
        );
        assert_eq!(msg.get(2), Some(&Attribute::Fingerprint(0xc8fb_0b4c)));
        ...
        assert_eq!(msg.fingerprint_offset(), Some(76));
        assert!(msg.username().is_none());
        let _ = msg.clone();
    }
    ...
    fn rfc5769_2_3_verifies_with_a_short_term_key() {
        ...
        assert!(verify_integrity(&buf, loc.off, IntegrityAlgorithm::Sha1, SECRET).is_ok());
        assert!(verify_fp(&buf, 76).is_ok());
        let input = mac_input(&buf, loc.off, 20).unwrap();
        assert_eq!(input.len(), 60);
        assert_eq!(&input[LENGTH_OFFSET..4], &[0x00, 0x40]);
        assert!(fingerprint_off(&buf) == Some(76));
        assert!(AttributeLocation { off: loc.off, code: loc.code, value_len: loc.value_len }
            == loc);
    ...
    fn rfc5769_2_4_verifies_with_a_long_term_key() {
        ...
        assert_eq!(
            key,
            [
                0xe8, 0xca, 0x7a, 0xd5, 0x9d, 0x5e, 0xb0, 0x51, 0x8e, 0x31, 0x29, 0x11, 0x2d,
                0xda, 0xb2, 0xa9
            ]
        );
    ...
    fn parse_rejects_trailing_octets() {
        ...
        let t = txid.as_bytes();
        let unknown = Bytes::copy_from_slice(&[1u8, 2, 3]);
        let mut body = Vec::new();
        emit_attribute(&mut body, AttributeKind::Unknown(0x8021), &unknown).unwrap();
        let trailing = body[..3].to_vec();
        ...
        let mut datagram2 = build_datagram(0x0000, t, &body[..20], None, false);
        let bad_length = 13u16.to_be_bytes();
        datagram2[LENGTH_OFFSET..4].copy_from_slice(&bad_length);
        assert_eq!(parse(&datagram2).unwrap_err().kind, ErrorKind::TrailingOctets);
    }
    ...
    fn parse_rejects_truncated_attributes() {
        ...
        assert_eq!(parse(&short).unwrap_err().kind, ErrorKind::TruncatedAttribute);
        let mut short2 = datagram.clone();
        short2.truncate(HEADER_LEN + 6);
        assert_eq!(parse(&short2).unwrap_err().kind, ErrorKind::TruncatedAttribute);
        let mut bad = datagram.clone();
        let bumped = bad[LENGTH_OFFSET + 1] + 4;
        bad[LENGTH_OFFSET + 1] = bumped;
    ...
    fn unknown_comprehension_optional_attributes_are_preserved() {
        ...
        assert_eq!(msg.len(), 1);
        assert!(!msg.is_empty());
        assert_eq!(msg.get(0), Some(&Attribute::Username("evtj:h6vY".to_string())));
    ...
    fn integrity_and_fingerprint_tampering_is_detected() {
        ...
        assert!(verify_fp(&buf, 64).is_err());
        let err = verify_fp(&buf, 64).unwrap_err();
    ...
    fn a_built_message_round_trips_through_parse() {
        ...
        let datagram = build_datagram(0x0000, t, &body_bytes, Some(SECRET), true);
        let msg = parse(&datagram).unwrap();
        ...
        assert!(msg.fingerprint() == Some(msg.find(FINGERPRINT_CODE).unwrap().to_value(t).unwrap()[0] as u32));
        let _ = msg.clone();
    }
    ...
    fn malformed_attributes_are_rejected() {
        ...
        assert_eq!(crate::attribute::attr_size(9), 12);
    ...
    fn message_integrity_sha256_length_rules_are_enforced() {
        let txid = TransactionId::default();
        let t = txid.as_bytes();
        let ok = build_datagram(
            0x0000,
            t,
            &body(&Attribute::Software("s".to_string()), t),
            None,
            false,
        );
        ...
        assert!(AttributeLocation { off: 20, code: 0x001c, value_len: 20 }.algorithm().is_none());
    ...
    fn constants_match_rfc8489() {
        ...
        assert_eq!(integrity_off(&RFC5769_2_1).unwrap().off, 80);
        ...
        assert_eq!(fingerprint_off(&RFC5769_2_1), Some(104));
        assert_eq!(fingerprint_off(&RFC5769_2_2), Some(64));
        assert_eq!(fingerprint_off(&RFC5769_2_3), Some(76));
