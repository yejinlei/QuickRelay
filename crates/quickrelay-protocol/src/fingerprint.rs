...
pub fn compute(datagram: &mut [u8], fp_off: usize) -> Result<Fingerprint, Error> {
    ...
    // The attribute is a 4-octet header plus its 4-octet value.
    if fp_off < HEADER_LEN || fp_off + 8 > datagram.len() {
        return Err(Error::attr(ErrorKind::MalformedAttribute, FINGERPRINT_ATTR));
    }
    let recorded = u16::from_be_bytes([datagram[2], datagram[3]]);
    if usize::from(recorded) + HEADER_LEN != datagram.len() {
...
pub fn verify(datagram: &[u8], fp_off: usize) -> Result<(), Error> {
    ...
    // The length field must be correct, not zeroed: unlike MESSAGE-INTEGRITY,
    // FINGERPRINT covers it (RFC 8489 Section 14.7).
    let recorded = u16::from_be_bytes([datagram[2], datagram[3]]);
    if usize::from(recorded) + HEADER_LEN != datagram.len() {
        return Err(Error::new(ErrorKind::LengthMismatch));
    }
    ...
    let equal = Into::<bool>::into(expected.bits().ct_eq(&stored.bits()));
    if !equal {
        return Err(Error::attr(ErrorKind::FingerprintMismatch, FINGERPRINT_ATTR));
    }
    Ok(())
}
...
mod tests {
    ...
    fn verify_rejects_a_corrupt_fingerprint() {
        // Any prefix octet breaks the CRC: the whole 20-octet header is in it.
        let mut msg = RFC5769_2_2;
        msg[0] = 0x02;
        assert_eq!(verify(&msg, 72).unwrap_err().kind, ErrorKind::FingerprintMismatch);
        // ...and any octet of the preceding attribute is too.
        let mut msg3 = RFC5769_2_2;
        msg3[40] ^= 0x80;
        assert_eq!(verify(&msg3, 72).unwrap_err().kind, ErrorKind::FingerprintMismatch);
        // A tampered stored value is caught as well: the four value octets
        // hold the CRC, so changing one makes it disagree with the prefix.
        let mut msg2 = RFC5769_2_2;
        msg2[76] ^= 0x01;
        assert_eq!(verify(&msg2, 72).unwrap_err().kind, ErrorKind::FingerprintMismatch);
        let mut msg4 = RFC5769_2_2;
        msg4[79] ^= 0x80;
        assert_eq!(verify(&msg4, 72).unwrap_err().kind, ErrorKind::FingerprintMismatch);
    }
    ...
    fn length_field_must_be_correct_not_zeroed() {
        ...
        let mut msg = RFC5769_2_2;
        msg[2..4].fill(0);
        // Zeroed length means the datagram no longer matches its own
        // declaration, so it is rejected as a length error before the CRC is
        // compared. With the correct length, the same prefix must still be
        // covered, so a changed prefix is a genuine mismatch.
        assert_eq!(verify(&msg, 72).unwrap_err().kind, ErrorKind::LengthMismatch);
    ...
    fn short_and_mismatched_datagrams_are_rejected() {
        let short = [0u8; 12];
        // Twelve octets is not even a full STUN header.
        assert_eq!(verify(&short, 0).unwrap_err().kind, ErrorKind::TooShort);
        assert_eq!(compute(&mut [0u8; 12], 0).unwrap_err().kind, ErrorKind::TooShort);
        // A header with no FINGERPRINT attribute: malformed, not too short.
        assert_eq!(verify(&[0u8; 20], 0).unwrap_err().kind, ErrorKind::MalformedAttribute);
        assert_eq!(compute(&mut [0u8; 20], 0).unwrap_err().kind, ErrorKind::MalformedAttribute);
        // A too-large length field fails before the CRC is compared.
        let mut msg = RFC5769_2_2;
        msg[3] = 0x3d;
        assert_eq!(verify(&msg, 72).unwrap_err().kind, ErrorKind::LengthMismatch);
        assert_eq!(compute(&mut msg, 72).unwrap_err().kind, ErrorKind::LengthMismatch);
        // A too-small length field fails the same way.
        let mut msg2 = RFC5769_2_2;
        msg2[3] = 0x3b;
        assert_eq!(verify(&msg2, 72).unwrap_err().kind, ErrorKind::LengthMismatch);
    }
