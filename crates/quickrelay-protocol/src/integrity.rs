...
pub fn compute(datagram: &mut [u8], attr_off: usize, algorithm: IntegrityAlgorithm, key: &[u8]) -> Result<[u8; 32], Error> {
    ...
    // `tag` is a 32-octet buffer of which only the first `tag_len` octets are
    // the MAC: an SHA-1 integrity is 20 octets long, so copying the whole
    // buffer into the attribute value would write 12 octets past the end.
    let n = tag_len(mac_len);
    datagram[attr_off + 4..need].copy_from_slice(&tag[..n]);
    Ok(tag)
}
...
pub fn mac_input(datagram: &[u8], attr_off: usize, mac_len: usize) -> Result<Vec<u8>, Error> {
    ...
    if attr_off + 4 + mac_len > crate::fingerprint::MAX_DATAGRAM {
...
mod tests {
    ...
    fn wrong_key_is_rejected() {
        ...
        // A one-octet difference anywhere in the message fails. Octets of the
        // attribute value are skipped: MAC input stops at the header.
        for at in [0usize, 20, 40, 75] {
        ...
        // The magic cookie is inside the MAC input too. The message-length
        // field is covered by the HMAC as well, but this crate checks it
        // against the buffer first, so a tampered length field is reported as
        // a length error rather than an integrity mismatch.
        let mut msg = RFC5769_2_1;
        msg[5] ^= 0x01;
        assert_eq!(
            verify(&msg, loc.off, IntegrityAlgorithm::Sha1, SECRET).unwrap_err().kind,
        ...
        let mut msg2 = RFC5769_2_1;
        msg2[2] ^= 0x01;
        assert_eq!(
            verify(&msg2, loc.off, IntegrityAlgorithm::Sha1, SECRET).unwrap_err().kind,
