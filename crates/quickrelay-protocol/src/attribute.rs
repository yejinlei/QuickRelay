...
/// Whether an attribute must be the final attribute in the message.
#[inline]
...
pub mod error_code {
    ...
    /// 388, Unknown Attribute. The RFC 5389 Section 12.2 code, renumbered to
    /// `420` by RFC 8489 Section 12.2; retained for reading messages from
    /// RFC 5389-era peers.
    ...
    /// 420, Unknown Attribute. RFC 8489 Section 12.2 renumbers the RFC 5389
    /// `388 Unknown Attribute` code to `420`.
...
impl AttrCode {
    ...
    pub fn name(self) -> &'static str {
        match self {
            ...
            AttrCode::IcePriority => "ICE-PRIORITY",
...
pub fn parse_attribute_value(
    ...
    match code {
        ...
        AttrCode::MessageIntegritySha256 => {
            check_exact(code, buf, MESSAGE_INTEGRITY_SHA256_LEN)?;
            let mut tag = [0u8; MESSAGE_INTEGRITY_SHA256_LEN];
            tag.copy_from_slice(buf);
...
pub fn write_attribute_value(
    ...
    match attr {
        ...
        Attribute::UnknownAttributes(list) => {
            ...
            let n = write_exact(out, &list.iter().flat_map(|c| c.to_be_bytes()).collect::<Vec<_>>())?;
            Ok(n)
        }
...
mod tests {
    ...
    fn all_codes_roundtrip() {
        ...
        assert_eq!(AttrCode::all().count(), 41);
    }

    #[test]
    fn unregistered_code_is_unknown() {
        for code in [
            0x0000u16, 0x0002, 0x0003, 0x0004, 0x0005, 0x0007, 0x000B, 0x000E, 0x000F,
    ...
    fn error_code_helpers_are_correct() {
        ...
        assert_eq!(error_code::unknown_attribute().as_u16(), 388);
    ...
    fn fixed_width_value_roundtrips() {
        ...
        assert_eq!(
            parse_attribute_value(AttributeKind::Known(AttrCode::Lifetime), &[0, 0x01, 0x2c, 0], &TXID).unwrap(),
            Attribute::Lifetime(300)
        );
        assert_eq!(
            parse_attribute_value(AttributeKind::Known(AttrCode::ResponsePort), &[0x0d, 0xba], &TXID).unwrap(),
    ...
    fn reserved_code_points_are_not_buildable() {
        // 0x0002 (was RESPONSE-ADDRESS) and 0x0003 (was CHANGE-REQUEST) are reserved.
        assert_eq!(AttrCode::attr_from_wire(0x0002), AttributeKind::Unknown(0x0002));
        assert_eq!(AttrCode::attr_from_wire(0x0003), AttributeKind::Unknown(0x0003));
        let v = parse_attribute_value(AttributeKind::Unknown(0x0003), &[0x01, 0x02], &TXID).unwrap();
        assert_eq!(v, Attribute::Unknown { kind: 0x0003, value: vec![1, 2] });
    }
    ...
    fn emit_attribute_pads_to_boundary() {
        ...
        assert_eq!(out.len(), 8, "4-byte header + 3 octets + 1 pad octet");
        emit_attribute(&mut out, AttributeKind::Known(AttrCode::Fingerprint), &[0, 0, 0, 0]).unwrap();
        assert_eq!(out.len(), 12, "second attribute is exactly 8 octets");
        emit_attribute(&mut out, AttributeKind::Known(AttrCode::DontFragment), b"").unwrap();
        assert_eq!(out.len(), 16, "header-only attribute occupies 4 octets");
    }
    ...
    fn names_are_non_empty() {
        ...
        assert!(AttrCode::all().all(|c| c.name().chars().all(|c| c.is_ascii_uppercase() || c == '-')));
    }
